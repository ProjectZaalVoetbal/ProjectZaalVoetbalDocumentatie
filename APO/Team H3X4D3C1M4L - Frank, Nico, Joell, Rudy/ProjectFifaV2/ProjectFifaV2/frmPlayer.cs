...
namespace ProjectFifaV2
{
    public partial class frmPlayer : Form
    {
        ...
        private void btnClearPrediction_Click(object sender, EventArgs e)
        {
            ...
            {
                foreach (Control cur in pnlPredCard.Controls)
                {
                    if (cur.GetType() == typeof(System.Windows.Forms.TextBox))
                        cur.Text = "0";
                }
                btnEditPrediction.PerformClick();
            }
        }
        ...
        private void btnEditPrediction_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < pnlPredCard.Controls.Count; i++)
            {
                ...
                {
                    if (i % 4 == 1) 
                        predictions.Add(int.Parse(pnlPredCard.Controls[i].Text) + " " + pnlPredCard.Controls[i - 1].Text); 
                    else if (i % 2 == 1) 
                        predictions[predictions.Count - 1] += " " + (int.Parse(pnlPredCard.Controls[i - 1].Text) + " " + pnlPredCard.Controls[i].Text); 
                }
                catch (Exception ex)
                {
                    completed = false;

                    if (MessageBox.Show("Unable to save\n\n" + ex.Message, "An error has occured", MessageBoxButtons.RetryCancel, MessageBoxIcon.Error) == System.Windows.Forms.DialogResult.Cancel)
                        break;
                    btnEditPrediction.PerformClick();
                }
            }
            ...
            if (completed)
            {
                StreamWriter sw = new StreamWriter(@"C:\Users\Frank\AppData\Roaming\Hexadecimal\save" + userName + ".hex");
                sw.WriteLine(userName);
                foreach (string cur in predictions) { sw.WriteLine(cur); }
                sw.Close();
            }
        }
